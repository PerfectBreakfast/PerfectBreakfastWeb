...
const CreateManagementUser = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const { managementUnitId } = location.state || {};
  ...
  useEffect(() => {
    const fetchRole = async () => {
      try {
        const result = await roleAPI.getRole(managementUnitId);
        setRoles(result);
      } catch (error) {
  ...
  const handleSubmit = async (event) => {
    ...
    data.append("managementUnitId", managementUnitId);
